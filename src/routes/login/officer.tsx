      ...
      if (isRegister) {
        try {
          const apiRes = await fetch("http://localhost:5000/api/auth/register", {
            ...
            body: JSON.stringify({ name, email: cleanEmail, password, role: "officer" })
          });
          if (apiRes.ok) {
          ...
          resData = await storeRegisterUser({ name, email: cleanEmail, password, role: "officer" });
        }

        toast.success("Officer Account Created!", {
          description: `Welcome to Government Officer workspace, ${resData.user.name}!`,
        });
      } else {
        try {
          const apiRes = await fetch("http://localhost:5000/api/auth/login", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ email: cleanEmail, password, role: "officer" })
          });
          if (apiRes.ok) {
            resData = await apiRes.json();
          } else {
          ...
          resData = await storeLoginUser({ email: cleanEmail, password, role: "officer" });
        }
